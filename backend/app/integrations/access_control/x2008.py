...
from app.integrations.base import AccessDecision, AccessEvent, IntegrationDisabled
...
# --------------------------------------------------------- normalized pipeline
#
# The two stages between "ADMS adapter" and "attendance write" in the
# architecture this integration follows:
#
#     X2008 -> ADMS -> [this adapter] -> AccessEvent -> [Member Resolver] ->
#     attendance_service.record_fingerprint_scan -> Owner/Trainer/Member
#
# Kept here (not in `hardware.py`) because both are specific to how the
# X2008/ADMS identifies a person and a device — a future terminal with a
# different identity scheme would supply its own versions of both, still
# producing the same `AccessEvent` shape for whatever comes after.


def to_access_event(
    record: ParsedFingerprintRecord, *, device_serial: str, received_at: datetime
) -> AccessEvent:
    """Normalize one parsed ADMS record — nothing added, nothing guessed.

    ``event_type`` is left unset: the X2008 is a passive attendance
    recorder with no in/out signal of its own, so asserting one here would
    be inventing a fact the device never sent. Direction is decided later,
    downstream, from the member's current state.
    """
    return AccessEvent(
        device_id=device_serial,
        device_user_id=record.enrolled_id,
        device_occurred_at=record.device_time,
        received_at=received_at,
        event_type=None,
        reference_id=f"{device_serial}:{record.enrolled_id}:{record.device_time_raw}",
        raw={"status_raw": record.status_raw, "verify_type_raw": record.verify_type_raw},
    )


def resolve_member(db: Any, *, device_id: int, device_user_id: str) -> Any:
    """The Member Resolver stage: a device's own enrolled-id, mapped to a
    GymFlow member — or ``None``.

    ``None`` is the whole point of this function existing on its own rather
    than being inlined at the call site: an enrolled-id this device has never
    had registered to a member must read as *unresolved*, plainly, so a
    caller cannot reach for a name-matching or phone-matching fallback
    without deliberately writing one — there isn't one to fall into by
    accident.
    """
    from app.db.models import FingerprintEnrollment, Member

    enrollment = db.scalar(
        select(FingerprintEnrollment).where(
            FingerprintEnrollment.device_id == device_id,
            FingerprintEnrollment.enrolled_id == device_user_id,
        )
    )
    if enrollment is None:
        return None
    return db.get(Member, enrollment.member_id)
